            ...
            db.collection("users").document(senderId).get().addOnSuccessListener(documentSnapshot -> {
                if (documentSnapshot.exists()) {
                    ...
                    if (usermodel != null) {
                        ...
                        Glide.with(context).load(usermodel.getProfilePicUrl()).circleCrop().placeholder(R.drawable.baseline_account_circle_24).error(R.drawable.profile).into(holder.profilePic);
                    }
                }
            });
        } else {
