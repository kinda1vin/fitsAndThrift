...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(userId)) {
            Log.e("ChatActivity", "User ID is null or empty");
            Toast.makeText(this, "User data is not available", Toast.LENGTH_SHORT).show();
            finish();
            return;
        }

        otherUser = new Usermodel(userId, username, email, profilePicUrl);

        chatroomId = Util.getChatroomId(Util.currentUserId(), userId);
        ...
        loadUserProfilePicture(profilePicUrl);
